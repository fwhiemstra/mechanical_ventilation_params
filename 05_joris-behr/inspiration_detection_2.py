...
def inspiration_detection_2(flow,rr):
    ...
    for i, idx in enumerate(idx_peak):
        ...
        #nog toevoegen, wat als waarde is negatief --> vooruit zoeken
        #daarna nogmaals kijken of er waardes vaak te dicht op elkaar liggen
        


    
    # Calculating end inspiratory 
    t_insp_est = (0.5*np.diff(start_insp)).astype(int)
    t_blank = 20
    end_insp = list()
    ...
    # for idx,val in enumerate(start_insp[0:-1]):
    #     a = find_nearest(flow[val+t_blank:val+t_insp_est[idx]],0)+val+t_blank
    #     # if flow[a]+50 < -100:
    #     end_insp.append(a)
    #     # else:
    #     #     end_insp.append(find_nearest(flow[a+1:a+t_insp_est[idx]],0)+a)

    for idx, val in enumerate(start_insp[0:-1]):
        # print(range(val+t_blank,val+t_insp_est[idx]))
        for i in range(val+t_blank,val+t_insp_est[idx]):
    ...
    ax1.scatter(np.asarray(time_sec)[idx_peak],flow[idx_peak])
    ax2.scatter(np.array(time_sec)[idx_peak],flow_diff[idx_peak])
