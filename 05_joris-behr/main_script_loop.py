...
from statistics_calculator import sd_se_statistics, correlations, correlations_xlsx_df
...
annotation = 0
params = ['234', 2, 'test']
insp_detection = 'script1'
insp_comp = ''
output_xlsx_file = []
input_folder = r'C:\Users\joris\OneDrive\Documenten\Studie\TM jaar 2&3\Q1\data\wave_mode'
pcw_list= []


pat_list = [r"1\Waves_001.txt", r"2\Waves_002.txt",r"3\Waves_003.txt", r"4\Waves_004.txt",r"5\Waves_005.txt", r"6\Waves_006.txt", r"7\Waves_007.txt", r"8\Waves_008.txt"]#,r"9\Waves_009.txt"]#, r"10\Waves_010.txt",r"11\Waves_011.txt"]
params_tot = pd.DataFrame()

end_insp_val_tot =[]
start_insp_val_tot = []
for pat in pat_list:
    input_file = os.path.join(input_folder,pat)
    ...
    # """Calculating random pcw to calculate pmus"""
    ...
    ## part to combine all parameters and start- and end-inspiration flow values
    if params_tot.empty:
        params_tot = param
    else: 
        params_tot= pd.concat([params_tot,param])
    
    if len(start_insp_val_tot) == 0:
        start_insp_val_tot = start_insp_values
        end_insp_val_tot = end_insp_values
    else:
        start_insp_val_tot = np.hstack((start_insp_val_tot,start_insp_values))
        end_insp_val_tot = np.hstack((end_insp_val_tot,end_insp_values))

# print(params_tot.head())
matrix_tot, matrix_aw, matrix_es, matrix_tp = correlations_xlsx_df(params_tot)
plt.show()