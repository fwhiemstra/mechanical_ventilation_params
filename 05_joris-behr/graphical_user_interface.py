...
def graphical_user_interface(annotation):
    """ Returns:
    ...
    if annotation == 1:
        def input2_():
            filename = filedialog.askopenfilename(title="Select annotation file",
                                                  filetypes=(("executables", "*.txt"),
                                                         ("all files", "*.*")))
            input_filename_annotation.append(filename)
            if filename == '' or filename == []:  # if entry canceled or left empty
                tk.messagebox.showwarning(
                    title="Input Warning", message="Annotation file is missing, no artefact scoring possible")
                annotation = 0
            else:
                print(filename)
                for name in input_filename_annotation:
                    label = tk.Label(frame, text=name, bg="#009fbd")
                    label.pack()
    ...
    def output_():
        ...
        if filename == '' or filename == []:  # if entry canceled or left empty
            tk.messagebox.showerror(
                title="Input Error", message="xxx Output file is missing. Please try again.")
    ...
    def get_variables():
        ...
        if patient_number_entry.get() == '':
            ...
            if outputfile_var.get() != OUTPUT_FILE.EXISTING_FILE and \
                ...
                tk.messagebox.showerror(
                    title="Input Error", message="yy Output file is missing. Please try again.")
    ...
    # input file selection
    if annotation == 1:
        inputfileannotation_title = tk.Label(frame, text="Select annotated input file * :", fg="#009fbd")
        inputfileannotation_title.pack()
        inputfileannotation = tk.Button(frame, text="Select annotated input file", padx=10, pady=5, fg="white",
                            bg="#003c7d", command=input_)
        inputfileannotation.pack()
    ...
    # End tkinter module
    root.mainloop()
    return annotation, patient_number, params, input_filename, input_filename_annotation, output_filename
